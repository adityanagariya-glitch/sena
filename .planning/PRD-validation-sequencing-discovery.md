...
The authoritative reference for every validator's rule and error string is the **Flutter code itself** — `lib/core/utils/validators.dart` plus the per-step controllers under `lib/features/client/.../client_step{1..6}_*controller.dart`, with error text resolved through `lib/core/constants/app_strings.dart`. The reference doc at `.claude/client_onboarding_validations.md` is a hand-maintained companion that captures intent, but **when the doc and the Flutter code disagree, the Flutter code wins**. The server implementation MUST mirror the Flutter code byte-for-byte; the doc is updated to match the code, never the other way around.

This direction was reversed on 2026-05-07 after user feedback. The earlier policy ("doc is canon") was wrong: the user-facing app already ships with the Flutter validator strings in production, and the assistant's voice prompts must match what the participant would have read on screen. Re-typing the doc text in the assistant's mouth would create a second source of truth and confuse participants who switch between voice and visual modes.

Cross-check (2026-05-07) found 58/62 fields where doc and code already match, 1 intentional FAIL (emergency-contact phone equality is not enforced — code is correct, doc agrees), and 3 wording-only DRIFTs (funding allocation, medical history year range, one more). Under the new policy the server mirrors the Flutter `AppStrings` text for the 3 DRIFT fields and the doc gets corrected as a follow-up. Full report: `.planning/VALIDATION-CROSS-CHECK-2026-05-07.md`.
