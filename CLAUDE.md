...
## todos
```
sena-ai\services\onboarding\src\onboarding\services\gemini_live.py:799  # TODO: Phase 1.5: thread from assert_session_owner
```
...
h3 Per-role access control — voice-mutable (appears after `selected_roles` is set)
h4 Per-role sub-fields
h4 Example `update_field` calls
h1 Set allowed_information for Manager role:
h1 Set purpose for Manager:
h1 Set timeframe for Manager:
h1 Set timeframe to until a date, then set the date:
h4 Dialogue flow for per-role fields
...
def emit_usage(*, tenant_id: str, user_id: str | None, feature: UsageFeature, model: str, session_id: str | None, prompt_tokens: int, response_tokens: int, cached_tokens: int, audio_seconds_in: float, audio_seconds_out: float, tool_call_count: int, latency_ms: int | None, success: bool, failure_reason: str | None, **extras: Any) → None
```
