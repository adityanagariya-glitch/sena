# scripts/registry.py
# Shared DynamoDB registry functions for document management.
# Used by FastAPI admin endpoints.
# No Lambda dependency.
...
REGISTRY_TABLE = "sena-doc-registry"
...
def registry_get(doc_id: str) -> dict | None:
    """Gets a single registry entry by doc_id."""
    table = dynamodb.Table(REGISTRY_TABLE)
    try:
...
def registry_list_by_org(org_id: str) -> list:
    """Lists all registry entries for a given org using GSI."""
    table = dynamodb.Table(REGISTRY_TABLE)
    try:
